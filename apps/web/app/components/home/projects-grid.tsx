    ...
    <div className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
      {isLoading
        ? Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="glass h-[260px] animate-pulse rounded-2xl" />
          ))
        : filteredProjects.map((project) => <ProjectCard key={project.id} project={project} />)}

      <Link
        to="/editor/new"
        className="glass flex min-h-[200px] items-center justify-center gap-2 rounded-2xl border-dashed text-[13px] font-medium text-[var(--mr-mfg)] hover:border-[var(--mr-acc)] hover:text-[var(--mr-fg)]"
      >
